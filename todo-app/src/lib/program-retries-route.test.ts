import { describe, it, expect } from "vitest";
import {
  buildProgramRetriesResponse,
  type ProgramRetriesRequestBody,
} from "../app/api/metrics/program-retries/route";

const AUDIT_PROJECT = "a98e9be3-679b-4469-94e2-07ad22b6deac";
const NOW = "2026-06-25T00:00:00Z";
const dayAgo = (d: number) =>
  new Date(new Date(NOW).getTime() - d * 24 * 60 * 60 * 1000).toISOString();

describe("program.retries rollup surface (buildProgramRetriesResponse)", () => {
  it("reproduces the audit finding: project=a98e..., 30d_count=2", () => {
    const body: ProgramRetriesRequestBody = {
      now: NOW,
      nodes: [
        {
          project_id: AUDIT_PROJECT,
          task: "worker [RETRY attempt=1/3] step",
          created_at: dayAgo(2),
        },
        {
          project_id: AUDIT_PROJECT,
          task: "worker [RETRY attempt=2/3] step",
          created_at: dayAgo(1),
        },
        {
          project_id: AUDIT_PROJECT,
          task: "non retry task",
          created_at: dayAgo(1),
        },
        {
          // outside the 30d window: must be excluded
          project_id: AUDIT_PROJECT,
          task: "stale [RETRY attempt=9/9]",
          created_at: dayAgo(60),
        },
      ],
    };
    const out = buildProgramRetriesResponse(body);
    expect(out.metric).toBe("program.retries");
    expect(out.windowDays).toBe(30);
    expect(out.total).toBe(2);
    expect(out.byProject[AUDIT_PROJECT]).toBe(2);
  });

  it("reproduces audit fingerprint fdb94a636eb04296: 30d_count=3 over rollup surface", () => {
    const body: ProgramRetriesRequestBody = {
      now: NOW,
      nodes: [
        { project_id: AUDIT_PROJECT, task: "a [RETRY attempt=1/3]", created_at: dayAgo(1) },
        { project_id: AUDIT_PROJECT, task: "b [RETRY attempt=2/3]", created_at: dayAgo(8) },
        { project_id: AUDIT_PROJECT, task: "c [RETRY attempt=3/3]", created_at: dayAgo(25) },
        { project_id: AUDIT_PROJECT, task: "old [RETRY attempt=1]", created_at: dayAgo(120) },
      ],
    };
    const out = buildProgramRetriesResponse(body);
    expect(out.total).toBe(3);
    expect(out.byProject[AUDIT_PROJECT]).toBe(3);
    expect(out.windowDays).toBe(30);
    expect(out.metric).toBe("program.retries");
  });

  it("reproduces audit fingerprint b24abf771af7eebe: 30d_count=4 over rollup surface", () => {
    const body: ProgramRetriesRequestBody = {
      now: NOW,
      nodes: [
        { project_id: AUDIT_PROJECT, task: "a [RETRY attempt=1/4]", created_at: dayAgo(1) },
        { project_id: AUDIT_PROJECT, task: "b [RETRY attempt=2/4]", created_at: dayAgo(6) },
        { project_id: AUDIT_PROJECT, task: "c [RETRY attempt=3/4]", created_at: dayAgo(15) },
        { project_id: AUDIT_PROJECT, task: "d [RETRY attempt=4/4]", created_at: dayAgo(28) },
        { project_id: AUDIT_PROJECT, task: "stale [RETRY attempt=9/9]", created_at: dayAgo(90) },
        { project_id: AUDIT_PROJECT, task: "plain task no marker", created_at: dayAgo(2) },
      ],
    };
    const out = buildProgramRetriesResponse(body);
    expect(out.metric).toBe("program.retries");
    expect(out.windowDays).toBe(30);
    expect(out.total).toBe(4);
    expect(out.byProject[AUDIT_PROJECT]).toBe(4);
  });

  it("reproduces audit fingerprint 2670f697533773db: 30d_count=5 over rollup surface", () => {
    const body: ProgramRetriesRequestBody = {
      now: NOW,
      nodes: [
        { project_id: AUDIT_PROJECT, task: "a [RETRY attempt=1/5]", created_at: dayAgo(1) },
        { project_id: AUDIT_PROJECT, task: "b [RETRY attempt=2/5]", created_at: dayAgo(5) },
        { project_id: AUDIT_PROJECT, task: "c [RETRY attempt=3/5]", created_at: dayAgo(12) },
        { project_id: AUDIT_PROJECT, task: "d [RETRY attempt=4/5]", created_at: dayAgo(20) },
        { project_id: AUDIT_PROJECT, task: "e [RETRY attempt=5/5]", created_at: dayAgo(28) },
        { project_id: AUDIT_PROJECT, task: "stale [RETRY attempt=9/9]", created_at: dayAgo(90) },
        { project_id: AUDIT_PROJECT, task: "plain task no marker", created_at: dayAgo(2) },
      ],
    };
    const out = buildProgramRetriesResponse(body);
    expect(out.metric).toBe("program.retries");
    expect(out.windowDays).toBe(30);
    expect(out.total).toBe(5);
    expect(out.byProject[AUDIT_PROJECT]).toBe(5);
  });

  it("returns an empty rollup shape when no nodes are supplied", () => {
    const out = buildProgramRetriesResponse({ nodes: [], now: NOW });
    expect(out).toEqual({
      metric: "program.retries",
      windowDays: 30,
      total: 0,
      byProject: {},
    });
  });

  it("honors a custom windowDays from the request body", () => {
    const out = buildProgramRetriesResponse({
      now: NOW,
      windowDays: 7,
      nodes: [
        { project_id: "p1", task: "[RETRY attempt=1]", created_at: dayAgo(3) },
        { project_id: "p1", task: "[RETRY attempt=2]", created_at: dayAgo(10) },
      ],
    });
    expect(out.windowDays).toBe(7);
    expect(out.total).toBe(1);
    expect(out.byProject).toEqual({ p1: 1 });
  });
});
